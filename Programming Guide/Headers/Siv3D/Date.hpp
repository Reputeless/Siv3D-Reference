...
namespace s3d
{
	...
	struct Date
	{
		...
		/// </returns>
		_Check_return_
		const String& dayOfWeekJP() const noexcept;
		...
		/// </returns>
		_Check_return_
		const String& dayOfWeekEN() const noexcept;
		...
		/// 日
		/// </param>
		explicit constexpr Date(int32 _year, _Field_range_(1, 12) int32 _month = 1, _Field_range_(1, 31) int32 _day = 1)
			: year(_year)
			, month(_month)
			, day(_day) {}
