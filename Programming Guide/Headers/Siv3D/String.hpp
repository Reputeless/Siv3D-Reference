...
namespace s3d
{
	...
	private:
		...
		std::basic_string<wchar> m_string;

	public:

		using string_type		= std::basic_string<wchar>;
		...
		/// 終端の L'\0' を含みません。
		/// </remarks>
		Property_Get(size_t, length) const noexcept { return m_string.length(); }

		/// <summary>
		/// 空の文字列であるかを示します。
		/// </summary>
		Property_Get(bool, isEmpty) const noexcept { return m_string.empty(); }
		...
		/// </param>
		/// <remarks>
		/// length が元の文字列の長さより小さい場合、元の文字列を返します。
		/// </remarks>
		/// <returns>
		/// 新しい文字列
		/// </returns>
		String padLeft(size_t length, wchar fillChar = L' ') const;

		/// <summary>
		/// 指定した長さになるよう、左に文字を詰めた文字列を返します。
		/// </summary>
		/// <param name="length">
		/// 新しい文字列の長さ
		/// </param>
		...
		/// 新しい文字列
		/// </returns>
		String padRight(size_t length, wchar fillChar = L' ') const;

		/// <summary>
		/// 先頭にある空白と制御文字を除去した文字列を返します。
		/// </summary>
		/// <remarks>
		/// 空白や制御文字が連続している場合は、それらはまとめて除去されます。
		/// </remarks>
		/// <returns>
		/// 新しい文字列
		/// </returns>
		...
		/// 見つかった文字の個数
		/// </returns>
		size_t count_if(std::function<bool(wchar)> function) const
		{
			return std::count_if(begin(), end(), function);
		}
		...
		/// 検索した文字が見つかった場合 true, それ以外の場合は false
		/// </returns>
		bool includes_if(std::function<bool(wchar)> function) const
		{
			return std::find_if(begin(), end(), function) != end();
		}
		...
		/// </returns>
		bool startsWith(wchar ch) const
		{
		...
		/// 置換後の文字列
		/// </returns>
		String replace(wchar oldChar, wchar newChar) const;
		...
		/// 新しい文字列
		/// </returns>
		String remove(const StringView& str) const;
		...
		/// 新しい文字列
		/// </returns>
		String remove_if(std::function<bool(wchar)> function) const;
...
namespace std
{
	inline void swap(s3d::String& x, s3d::String& y) noexcept(noexcept(x.swap(y)))
	{
		x.swap(y);
	}
