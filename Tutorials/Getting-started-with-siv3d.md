...
## １つだけ注意！
これから先に進むと登場する `Texture`, `DynamicTexture`, `Image`, `Sound`, `Wave`, `Font` といったデータは、画像や音楽を準備する処理に少し時間がかかります。  
...
Siv3D は文字の型に「ワイド文字」を使います。ワイド文字は
```cpp
L"あいうえお", L"Hello, world!", L'A', L'3'
```
のように、文字や文字列のクオーテーションの前に L を置きます。  
ワイド文字は、ひらがなや漢字を wchar 型 1 要素で表現できる利点があります。  
ワイド文字を使うことで日本語を含むテキストを、プログラムで簡単に扱えるようになります。  
